...
Do you mean pidgin Danish, perhaps? :''', '')
    ...
    def test_punctuation1(self):
        ...
        tts_items = t.prepare_item(tts_item)
        self.assertEqual(
            tts_items[0].text, 'Sure, these were all hoary old tropes even by the late \'70s/early \'80s, but this particular combination of them, at this exact date?')
        self.assertEqual(tts_items[1].text, 'Even the')
        self.assertEqual(tts_items[2].text, 'silly but kind of grim')
        self.assertEqual(
            tts_items[3].text, 'atmosphere that you correctly mention the game possessing is the exact way I would describe')
    ...
    def test_new_break3(self):
        ...
        tts_items = t.prepare_item(tts_items[0])
